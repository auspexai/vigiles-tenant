...
Knobs live in [`experiment.toml`](experiment.toml): `[experiment]`/`[executor]`/`[reducer]` feed the build, `[driver]` feeds the run. Requires `auspexai-tenant>=0.6.51` (the documented `--detach` concurrent flow); the `capture_*` profiles (D20 raw-content capture) require `>=0.6.60` — the release where a `[capture] raw` manifest builds at all (earlier validators rejected it), which also harvests the buffered raw live during the run and co-locates the exported bundle with it under `runs/<label>/`. The legacy `VIGILES_*` env vars still override the *driver* knobs (see [Long-horizon runs](#long-horizon-runs)).
